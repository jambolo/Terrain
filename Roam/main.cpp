/** @file *//********************************************************************************************************
	...
	$Header: //depot/Terrain/Roam/main.cpp#2 $
...
namespace
{
	...
	void Display();
	void Reshape( HWND hWnd, int w, int h );
	void InitializeRendering( HWND hWnd );
	void ShutDownRendering();
	void InitializeApp();
	void ShutDownApp();
//	void DrawTeapot();
	void FixCameraPosition( __int64 time );
	void UpdateMovement( __int64 time );

	void UpdateMouse( __int64 time );
	void ShutDownMouse();
	void InitializeMouse( HINSTANCE hInstance, HWND hWnd );

	char const			s_sAppName[]	= "Roam Terrain";
	char const			s_sTitleBar[]	= "Roam Terrain";

	DxObjects::TerrainCamera *	s_pCamera;
	Dxx::Camera *				s_pDebugCamera;
	float						s_CameraSpeed				= 1.5f;
	float						s_CameraTurnSpeed			= 90.0f;
	float						s_MouseSensitivity			= 0.2f;

	Hud *				s_pHud;

	DxObjects::Axes *	s_pAxes;
	DxObjects::SkyBox *	s_pSkyBox;
	Terrain *			s_pTerrain;

	IDirect3D9 *		s_pD3d;
	IDirect3DDevice9 *	s_pD3dDevice;
	D3DCAPS9			s_Caps;

//	ID3DXMesh *			s_pTeapot;
//	D3DMATERIAL9		s_TeapotMaterial	=
//						{
//							{ 1.0f, 1.0f, 1.0f, 1.0f },
//							{ 1.0f, 1.0f, 1.0f, 1.0f },
//							{ 1.0f, 1.0f, 1.0f, 1.0f },
...
//						};
	...
	bool	s_bFixCamera	= true;
	float	s_CameraHeight	= 2.0f;
	float	s_RecoveryRate	= 2.0f;
	bool	s_bDrawAxes		= false;

	LPDIRECTINPUT8			s_pDI;
	LPDIRECTINPUTDEVICE8	s_pMouseDevice;
